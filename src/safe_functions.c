...
/*   Updated: 2025/04/05 11:24:22 by dtanski          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	handle_mutex_error(int status, t_opcode opcode)
{
	...
	if (EINVAL == status && (LOCK == opcode || UNLOCK == opcode))
		...
		err_exit("A deadlock would occur if the thread blocked waiting for mutex");
	else if (EPERM == status)
		err_exit("The current thread does not hold a lock on mutex");
	else if (ENOMEM == status)
		err_exit("The process cannot allocate enough memory to create another mutex.");
