...
/*   Updated: 2025/04/07 16:29:38 by dtanski          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_output(t_status status, t_philo *philo)
{
	...
	if (status == SLEEPING && !get_bool(&philo->table->table_mtx, &philo->table->simulation_finished))
		...
		printf(R "%-6ld" " %d died\n" RST, timestamp, philo_id);
	else if ((status == GRAB_1ST_FORK || status == GRAB_2ND_FORK) && !get_bool(&philo->table->table_mtx, &philo->table->simulation_finished))
		printf(W "%-6ld" RST " %d has taken a fork\n" RST, timestamp, philo_id);
	// else
	// 	printf("Printing output failed.");
